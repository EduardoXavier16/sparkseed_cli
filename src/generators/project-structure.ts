...
function generateComponentTemplate(component: string, config: ProjectConfig): string {
...
function generateComponentStyles(component: string, config: ProjectConfig): string {
  const componentName = component.charAt(0).toUpperCase() + component.slice(1);
...
function generateComponentTest(component: string, config: ProjectConfig): string {
  const componentName = component.charAt(0).toUpperCase() + component.slice(1);
...
function generatePageTemplate(page: string, config: ProjectConfig): string {
...
function generateUseAuthHook(config: ProjectConfig): string {
  return `import { useContext } from 'react';\nimport { AuthContext } from '@/context/AuthContext';\n\nexport const useAuth = () => {\n  const context = useContext(AuthContext);\n  \n  if (!context) {\n    throw new Error('useAuth must be used within an AuthProvider');\n  }\n  \n  return context;\n};\n`;
}

function generateUseThemeHook(config: ProjectConfig): string {
  return `import { useContext } from 'react';\nimport { ThemeContext } from '@/context/ThemeContext';\n\nexport const useTheme = () => {\n  const context = useContext(ThemeContext);\n  \n  if (!context) {\n    throw new Error('useTheme must be used within a ThemeProvider');\n  }\n  \n  return context;\n};\n`;
}

function generateAuthContext(config: ProjectConfig): string {
...
function generateThemeContext(config: ProjectConfig): string {
...
function generateApiService(config: ProjectConfig): string {
...
function generateCssVariables(config: ProjectConfig): string {
...
function generateTypesIndex(config: ProjectConfig): string {
...
function generateBackendPackageJson(config: ProjectConfig): string {
  return JSON.stringify({
...
function generateBackendTsConfig(config: ProjectConfig): string {
  return JSON.stringify({
    compilerOptions: {
      target: 'ES2020',
      module: 'commonjs',
...
function generateAuthServiceBackend(config: ProjectConfig): string {
